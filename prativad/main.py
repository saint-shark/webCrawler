...
def get_all_articles(url, news_type, offset):
    # print (url)
    complete_url = url + news_type + "&pg=" + str(offset)
    # print (complete_url)
    try:
        soup = make_soup(complete_url)
        articles = soup.select('.article-header')
...
def get_article(url):
     article_desc = {}
     soup = make_soup(url)
     article_desc['title'] = soup.select('.content-article-title h2')[0].get_text()
     try:
         article_desc['image_url'] = "http://www.prativad.com" + soup.select('.setborder')[0].get('src')
     except:
         pass
    #  article_desc['news_time'] = soup.select('.entry-date')[0].contents[0]
     try:
         article_desc['body'] = soup.select('.shortcode-content p')[0].get_text()
     except:
         pass
     print (article_desc)
     return article_desc
...
def main():
     ...
     types = ['14', '16', '17', '18', '21']

     for link_type in types:
            offset = 1
            while offset < 10:

                article_urls = get_all_articles("http://www.prativad.com/news.php?newsidx=", link_type, offset)
                offset = offset + 1

                for article_url in article_urls:
                    comp_url = "http://www.prativad.com/" + article_url
                    # print (comp_url)
