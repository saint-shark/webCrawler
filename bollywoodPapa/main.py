...
def get_all_articles(url, news_type, offset):
    ...
    try:
        ...
        articles = soup.select('.post-box-title')
        all_articles = []
...
def get_article(url):
     article_desc = {}
     soup = make_soup(url)
     article_desc['title'] = soup.select('.name span')[0].get_text()
     try:
         article_desc['image_url'] = soup.select('.alignnone')[0].get('src')
     except:
         pass
     article_desc['news_time'] = soup.select('.tie-date')[0].get_text()

     bodytext = []
     for paragraphs in range (0,20):
         try:
             bodytext.append(soup.select('.entry > p > p')[paragraphs].get_text())
         except:
             pass
...
def main():
     ...
     for link_type in types:
            offset = 1
            while offset < 15:
                article_urls = get_all_articles("https://www.bollywoodpapa.com/category/", link_type, offset)
                offset = offset + 1
