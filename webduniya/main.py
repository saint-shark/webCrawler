...
def get_all_articles(url, news_type):
    complete_url = url + "/" + news_type + "-hindi-news"
    #print (complete_url)
    try:
        soup = make_soup(complete_url)
        articles = soup.select('.headingMain')
        all_articles = []
...
def get_article(url):
     article_desc = {}
     soup = make_soup(url)
     article_desc['title'] = soup.select('.headingMain')[0].contents[0]
     try:
         article_desc['image_url'] = soup.select('.imgCont')[0].get('src')
     except:
         pass
     article_desc['news_time'] = soup.select('.artPostDate')[0].contents[0]

     article_desc['body'] = soup.select('.aricleBodyMain')[0].get_text()
     print (article_desc)
     return article_desc
...
def main():
     ...
     types = ['international', 'national', 'regional', 'latest']
     for link_type in types:
         article_urls = get_all_articles("http://hindi.webdunia.com", link_type)
         for article_url in article_urls:
             #print (article_url)
             comp_url = "http://hindi.webdunia.com" + article_url
