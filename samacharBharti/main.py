...
def get_all_articles(url, news_type):
    ...
    try:
        ...
        articles = soup.select('.entry-title')
...
def get_article(url):
     article_desc = {}
     soup = make_soup(url)
     article_desc['title'] = soup.select('.entry-title')[0].contents[0]
     try:
         article_desc['image_url'] = soup.select('.entry-content p a')[0].get('href')
     except:
         pass
     article_desc['news_time'] = soup.select('.entry-date')[0].contents[0]
     try:
         article_desc['body'] = soup.select('.entry-content')[0].get_text()
     except:
         pass
     print (article_desc)
     return article_desc
...
def main():
     ...
     types = ['vishva-bharati', 'desh-bharti', 'sports-bhariti', 'bollywood-bharti', 'business-bharti']

     for link_type in types:
            offset = 1
            article_urls = get_all_articles("http://www.samacharbharti.in", link_type)
