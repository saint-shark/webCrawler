...
def get_all_articles(url, news_type, offset):
    ...
    try:
        ...
        articles = soup.select('.CatNewsFirst_FirstNews h1')
        all_articles = []
...
def get_article(url):
     article_desc = {}
     soup = make_soup(url)
     article_desc['title'] = soup.select('.storyheadline')[0].contents[0]
     try:
         article_desc['image_url'] = soup.select('.storyImg img')[0].get('src')
     except:
         pass
     article_desc['news_time'] = soup.select('.storydate')[0].contents[0]
     try:
         article_desc['body'] = soup.select('.storydetails')[0].get_text()
     except:
         pass
     print (article_desc)
     return article_desc
...
def main():
     ...
     types = ['world', 'india', 'sports', 'entertainment', 'business-economy']
     ...
     for link_type in types:
            ...
            while offset < 200:
                ...
                article_urls, length = get_all_articles("http://www.uniindia.com/news", link_type, offset)


                offset = length + offset

                for article_url in article_urls:
                    comp_url = "http://www.uniindia.com" + article_url
                    print (comp_url)
